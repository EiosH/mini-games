...
function controlLMove(){
    if(!pause){
        var i=1;
        for(i=1; i<10; ++i){
            // console.log("  "+whereCanTo(i));
            if(whereCanTo(i)!=0&&i==whereCanTo(i)+1){
            // console.log(d[i]);//可以走动的元素
            ...
            d[target_d]=x;
            //把目标大DIV设置为被点击的小DIV的编号
            document.getElementById("d"+x).style.left=d_posXY[target_d][0]+"px";
            document.getElementById("d"+x).style.top=d_posXY[target_d][1]+"px";
            //最后设置被点击的小DIV的位置，把它移到目标大DIV的位置
            }
            break;
            }
...
function controlUMove(){
    if(!pause){
        var i=1;
        for(i=1; i<10; ++i){
            // console.log("  "+whereCanTo(i));
            if(whereCanTo(i)!=0&&i==whereCanTo(i)+3){
            // console.log(d[i]);//可以走动的元素
            var target_d=whereCanTo(i);
            if( target_d != 0){
            var x = d[i];
...
function init(){
    document.onkeydown=function(){
        ...
        switch(e.keyCode){
        case 39: controlRMove();break;
        case 37: controlLMove();break;
        case 40: controlDMove();break;
        case 38: controlUMove();break;
        }
    }
}
...
window.onload=function(){
    init();
    reset();
    // alert(parseInt(Math.random()*(max-min+1)+min));
}
